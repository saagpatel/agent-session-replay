...
- **Decision Flight Deck v0** — drag-drop AFR archives (`trace.afr.jsonl` plus
  ...
  bundles. Replay previews also include an operator next-step hint for matched,
  hidden, missing, and blocked bundles, so the UI says whether to switch presets,
  inspect evidence, refresh, or resolve approval first. Copied decision notes
  include that replay scope, drift, and next-step hint, so handoffs preserve why
  a pasted command is runnable, hidden, stale, or gone. Action bundles and
  decision notes also include a compact finding-to-command
